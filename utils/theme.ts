...
export function ThemeManager() {
  ...
  // JANK: Porting in localesettings as a way to quickly change text based on dark mode
  ...
  const onThemeSettingChange = (themeSetting: IThemeSettingOptions) => {
    ...
    if (themeCurrent.value === 'dark') {
      localeSetting.value = localeSetting.value
        ? localeSetting.value.split('_dark')[0] + '_dark'
        : 'en_dark'
    } else {
      localeSetting.value = localeSetting.value
        ? localeSetting.value.split('_dark')[0]
        : 'en'
    }
  }
